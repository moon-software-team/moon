...
import { VLCConfig, VLCStatus } from '../types/vlc';
...
export class VLCController {
  ...
  constructor(config: VLCConfig) {
    ...
    this.title = 'Inconnu';
    this.baseUrl = `http://localhost:${this.config.httpPort}`;
  }
  ...
  async openFile(filePath: string, title: string): Promise<void> {
    ...
    this.vlcProcess = spawn(this.config.vlcPath!, args, {
      detached: false,
      stdio: 'pipe'
    });
  ...
  private async makeRequest(endpoint: string, params: Record<string, any> = {}): Promise<any> {
    return new Promise((resolve, reject) => {
      const url = new URL(`/requests/${endpoint}`, this.baseUrl);
